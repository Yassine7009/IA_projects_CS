    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [
            ...
            if not self.walls[nextx][nexty]:
                nextPos = (nextx, nexty)
                nextCorners = [
                    corners[i] for i in range(len(corners)) if corners[i] != nextPos
                ]

                nextState = (nextPos, nextCorners)

                successors.append((nextState, action, 1))  # every step costs 1
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    possibilities = [
        abs(c[0] - currentPos[0]) + abs(c[1] - currentPos[1]) for c in cornersLeft
    ] + [0]

    # the +[0] is to make sure the result is positive
    return max(possibilities)  # Default to trivial solution
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    possibilities = [
        mazeDistance(position, c, problem.startingGameState) for c in foodGrid.asList()
    ] + [0]
